...
public:
	...
	void                                                Unload();
	xbool                                               ChangeBindingActor(const xchar* szOldActorName, XEActor* pNewActor, const xchar* szSpecificNodeName = NULL, 
		const xchar* szSpecificNodeType = XESeqBindingActorInstance::NODE_TYPENAME, xbool bNodeTypeFuzzyFind = xfalse);//to find the binding actor INSTANCE-nodes with the specific node-type and actor name, and replace with the new actor. [CAN INDICATE A SPECIFC NODE NAME AS WELL.]
public:
